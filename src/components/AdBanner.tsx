...
// Matches your app's bg-background / border / muted color tokens
export function AdPlaceholder({ isPro, label }: { isPro: boolean; label?: string }) {
  ...
  return (
    <div className="w-full rounded-xl border border-border bg-secondary/20 py-4 px-4 flex items-center justify-between gap-4">
      <div>
        <p className="text-[10px] uppercase tracking-widest text-muted-foreground/50 mb-0.5">Advertisement</p>
        <p className="text-xs text-muted-foreground">{label || "Your ad here"}</p>
      </div>
      <Link to="/upgrade" className="shrink-0 inline-flex items-center gap-1.5 text-xs font-semibold text-primary hover:underline">
        <Zap className="h-3 w-3" /> Remove ads
      </Link>
    </div>
  );
}